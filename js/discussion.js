...
var _discFocusTrap = null;

function _releaseDiscModal() {
  if (_discFocusTrap) { _discFocusTrap.release(); _discFocusTrap = null; }
}
...
function _discMarkdown(text) {
  ...
  function stashSvg(svg) {
    // 安全处理：优先使用 DOMPurify (MPL-2.0/Apache-2.0) 进行权威过滤，
    // 这是 cure53 安全团队维护的工业级 XSS sanitizer，可彻底解决：
    // - L1: data:image/svg+xml URI 注入
    // - L2: 未闭合 <script> 残留
    // - L3: style 属性 CSS 注入
    // - H1: HTML 实体编码绕过（&#106;avascript:）
    // - SMIL 动画 javascript: 注入
    // - foreignObject 嵌套任意 HTML
    if (typeof window !== 'undefined' && window.DOMPurify && typeof window.DOMPurify.sanitize === 'function') {
      try {
        // 仅放行 SVG + SVG 滤镜白名单，禁用 style 属性（防 CSS 注入），
        // 禁用 foreignObject（可嵌入任意 HTML/script）
        ...
        console.warn('[discussion] DOMPurify sanitize failed, fallback to regex:', e);
      }
    }
    // 降级正则过滤（DOMPurify 未加载或异常时使用）
    svg = svg.replace(/<script[\s\S]*?<\/script>/gi, '');
    svg = svg.replace(/<script[^>]*>/gi, ''); // 移除未闭合的 <script>
    svg = svg.replace(/[\s\/"']on\w+\s*=\s*"[^"]*"/gi, '');
    svg = svg.replace(/[\s\/"']on\w+\s*=\s*'[^']*'/gi, '');
    svg = svg.replace(/[\s\/"']on\w+\s*=\s*[^\s>]+/gi, '');
    svg = svg.replace(/<set\s[^>]*attributeName\s*=\s*["']?(?:href|xlink:href)["']?[^>]*>/gi, '');
    svg = svg.replace(/<animate\s[^>]*attributeName\s*=\s*["']?(?:href|xlink:href)["']?[^>]*>/gi, '');
    svg = svg.replace(/<(?:set|animate)\s[^>]*to\s*=\s*["']?\s*javascript:[^"']*["']?[^>]*>/gi, '');
    svg = svg.replace(/<(?:set|animate)\s[^>]*to\s*=\s*["']?\s*data:(?!image\/)[^"']*["']?[^>]*>/gi, '');
    // L1 修复：data:image/svg+xml URI 可执行脚本，必须过滤
    svg = svg.replace(/<(?:set|animate)\s[^>]*to\s*=\s*["']?\s*data:image\/svg\+xml[^"']*["']?[^>]*>/gi, '');
    // L3 修复：移除 style 属性（CSS 注入风险）
    svg = svg.replace(/\sstyle\s*=\s*"[^"]*"/gi, '');
    svg = svg.replace(/\sstyle\s*=\s*'[^']*'/gi, '');
    svg = svg.replace(/\sstyle\s*=\s*[^\s>]+/gi, '');
    // 解码 HTML 实体后再过滤 javascript: 和 data:
    ...
    // L1 修复：data:image/svg+xml 也必须过滤（可携带脚本）
    svg = svg.replace(/(href|xlink:href)\s*=\s*["']?\s*data:(?!image\/(?!svg\+xml))/gi, '$1="');
    svg = svg.replace(/(href|xlink:href)\s*=\s*["']?\s*data:image\/svg\+xml/gi, '$1="');
    // 移除 foreignObject（可嵌入任意 HTML/script）
    svg = svg.replace(/<foreignObject[\s\S]*?<\/foreignObject>/gi, '');
    var idx2 = svgBlocks.length;
    svgBlocks.push(svg);
    return '\u0000SVG' + idx2 + '\u0000';
  }
  // ```svg ... ``` 或 ```xml ... ``` 代码块
...
function renderDiscussionPage(target) {
  ...
  // 消息区
  html += '<div class="discussion-messages" id="discussion-messages" role="log" aria-live="polite" aria-label="多智能体讨论"></div>';
...
function _openCustomAgentModal() {
  ...
  overlay.setAttribute('aria-label', '自定义智能体');
  ...
  function close() { _releaseDiscModal(); overlay.remove(); _renderAgentBar(); }
  ...
  overlay.querySelectorAll('[data-del-id]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      _releaseDiscModal(); overlay.remove(); _openCustomAgentModal();
    });
  });

  if (window.BioQuestA11y && typeof window.BioQuestA11y.trapFocus === 'function') {
    _discFocusTrap = window.BioQuestA11y.trapFocus(overlay, {
      onEscape: close,
      initialFocus: overlay.querySelector('#disc-ca-name') || overlay.querySelector('button')
    });
  }
