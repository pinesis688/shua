...
/* 安全：API Key 仅存页面内存，不落 localStorage（XSS 可读取 localStorage 一锅端）。
   localStorage 仅持久化 provider/model 等非敏感偏好；刷新页面后需重新输入 Key。 */
var _aiKeyMemory = '';
...
function _loadApiKeyConfig() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (stored.apiKey) {
        _aiKeyMemory = stored.apiKey;
        _saveApiKeyConfig({ provider: cfg.provider, apiKey: _aiKeyMemory, model: cfg.model });
      }
    }
  } catch (e) {}
  cfg.apiKey = _aiKeyMemory;
  return cfg;
}

function _saveApiKeyConfig(cfg) {
  _aiKeyMemory = (cfg && cfg.apiKey) || '';
  try {
