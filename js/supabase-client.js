...
async function getLeaderboard(tab, limit) {
  ...
  try {
    ...
    var query = sb.from('profiles')
      .select('id, username, display_name, bio_score, practice_count, total_answered, total_correct, accuracy, current_streak')
      .order(orderCol, { ascending: false })
      .not(orderCol, 'is', null)   // 排除 null：签到 0 天的用户 current_streak=null，不应排前
      .gt(orderCol, 0)              // 排除 0 值：未参与用户的 0 不应排前
      .limit(limit || 20);
    ...
    if (_currentUser && _currentUser.id) {
      try {
        var myProfile = await sb.from('profiles')
          .select('bio_score, total_answered, accuracy, current_streak')
